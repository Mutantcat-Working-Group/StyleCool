...
#### Style search endpoint `/skillapi`

- Returns style library results as JSON when the `token`, `q`, and `cat` parameters are present.
- Renders an HTML documentation page with built-in Chinese/English switching when parameters are absent.

#### Style example gallery `/example`

- 100 examples, each including full HTML, a thumbnail, and a copy-ready generation prompt.
- Supports `?q=` keyword search and `?p=` pagination.

#### Style library data

- Four platform categories: Web, Desktop, Mini Program, Mobile.
- Data files are decoupled from code; editing the JSON takes effect immediately.
